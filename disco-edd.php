...
class DiscoEdd {
	...
	public function assign_discourse_groups($download_id,$user_id){
		$discourse_groups = get_post_meta( $license->download_id, 'discourse_groups' );
		if(!empty($discourse_groups) && is_array($discourse_groups)){
			foreach ($discourse_groups as $discourse_group) {
				// Add/remove $payment->user_id from the group
			}
		}		
	}
